...
export class UserController {
  ...
  synchronize(
    ...
    return this.userService.synchronize(companyId, paginationDto)
    .then( (msg: string) => {
      const response = new PfxHttpResponseDto(HttpStatus.OK, msg);
      const end = performance.now();
  ...
  searchByValues(
    @Param('companyId', ParseUUIDPipe) companyId: string,
    @Query() paginationDto: SearchPaginationDto,
    @Body() inputDto: UserSearchInputDto
  ): Promise<PfxHttpResponseDto> {
    ...
    return this.userService.searchByValues(companyId, paginationDto, inputDto)
