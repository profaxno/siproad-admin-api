...
import { PermissionSearchInputDto } from './dto/permission-search.dto';
...
export class PermissionService {
  ...
  update(dto: PermissionDto): Promise<PermissionDto> {
    ...
    .then( (entity: Permission) => this.prepareEntity(entity, dto) )// * prepare
    .then( (entity: Permission) => this.save(entity) ) // * update
    .then( (entity: Permission) => {
      const dto = new PermissionDto(entity.name, entity.code, entity.id); // * map to dto
  ...
  remove(id: string): Promise<string> {
    ...
    .then( (entity: Permission) => this.save(entity) )
    .then( () => {
      const end = performance.now();
  ...
  searchByValues(companyId: string, paginationDto: SearchPaginationDto, inputDto: PermissionSearchInputDto): Promise<PermissionDto[]> {
    const start = performance.now();

    return this.searchEntitiesByValues(companyId, paginationDto, inputDto)
    .then( (entityList: Permission[]) => entityList.map( (entity) => new PermissionDto(entity.name, entity.code, entity.id) ) )
    .then( (dtoList: PermissionDto[]) => {
  ...
  findByIds(paginationDto: SearchPaginationDto, idList: string[]): Promise<Permission[]> {
    const {page=1, limit=this.dbDefaultLimit} = paginationDto;
  ...
  private prepareEntity(entity: Permission, dto: PermissionDto): Promise<Permission> {
  ...
  private searchEntitiesByValues(companyId: string, paginationDto: SearchPaginationDto, inputDto: PermissionSearchInputDto): Promise<Permission[]> {
    const {page=1, limit=this.dbDefaultLimit} = paginationDto;

    const query = this.permissionRepository.createQueryBuilder('a')
